...
echo Compiling Deep Forensic Diagnostic Harness...
cl %COMMON% /std:c++20 /EHsc %INCLUDES% /Febuild\diag_table_deep.exe tests_src\diag_table_deep.cpp core\roopm_render.cpp core\roopm_font.cpp core\roopm_image.cpp vendor\yoga\yoga\*.cpp vendor\yoga\yoga\algorithm\*.cpp vendor\yoga\yoga\node\*.cpp vendor\yoga\yoga\config\*.cpp vendor\yoga\yoga\debug\*.cpp vendor\yoga\yoga\event\*.cpp build\quickjs_unity.obj build\lexbor_unity.obj user32.lib advapi32.lib shell32.lib gdi32.lib /Fobuild\

if %ERRORLEVEL% EQU 0 (
    echo Running Diagnostic Engine...
    build\diag_table_deep.exe
)
