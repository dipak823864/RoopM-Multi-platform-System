...
cl %COMMON% /std:c++20 /EHsc %INCLUDES% /Febuild\test_layout_metrics.exe tests_src\test_layout_metrics.cpp core\roopm_render.cpp core\roopm_font.cpp core\roopm_image.cpp build\blend2d_unity.obj build\sizetable.obj build\checksum_sse4_2.obj build\otglyf_sse4_2.obj build\pixelconverter_ssse3.obj vendor\yoga\yoga\*.cpp vendor\yoga\yoga\algorithm\*.cpp vendor\yoga\yoga\node\*.cpp vendor\yoga\yoga\config\*.cpp vendor\yoga\yoga\debug\*.cpp vendor\yoga\yoga\event\*.cpp build\quickjs_unity.obj build\lexbor_unity.obj user32.lib advapi32.lib shell32.lib gdi32.lib shcore.lib /Fobuild\
if %ERRORLEVEL% NEQ 0 exit /b %ERRORLEVEL%

echo.
build\test_layout_metrics.exe
